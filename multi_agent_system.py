...
# ==========================================
# AATA V7.7 - Council Approved (Claude + ChatGPT + Gemini + Ahmida)
# Secure Version (Keys from Environment)
# ==========================================
...
def check_drawdown_protection(state, market):
    ...
    if not data_complete:
        print("Drawdown check skipped this cycle: incomplete market data for equity calculation.")
        return equity, None
...
class PaperBroker:
    ...
    def open_position(self, state, symbol, direction, risk_pct, strategy):
        ...
        if not math.isfinite(requested_risk) or requested_risk < 0:
            return {"ok": False, "error": "Invalid risk_pct (must be a finite non-negative number)"}
        ...
        log_to_supabase("OPEN_POSITION", {"symbol": symbol, "position": pos_data,
                                          ...
                                          "emotional_state": emo_state, "emotional_score": emo_score})

        note = ""
        if applied_risk < requested_risk:
            note = f" (طُلب {requested_risk*100:.1f}% لكن قُلّص إلى {applied_risk*100:.1f}% بسبب حالة: {emo_state})"
...
def get_portfolio_state() -> dict:
    ...
    if state is None:
        return {"ok": False, "error": "Memory corrupted - manual intervention required"}
    equity, data_complete = calculate_equity(state, market)
    effective_max, emo_state, emo_score = get_effective_max_risk(state)
    ...
    portfolio["note"] = f"حالتك الحالية: {emo_state}. الحد الأقصى المسموح الآن: {effective_max*100:.1f}% لكل صفقة، غير قابل للتجاوز."
    return portfolio
...
def open_position(symbol: str, direction: str, risk_pct: float, strategy: str) -> dict:
    ...
    if state is None:
        return {"ok": False, "error": "Memory corrupted - trade blocked for safety"}
...
def run_agent_cycle():
    ...
    sys_inst = ("أنت AATA، وكيل تداول ذكي. افحص المحفظة والسوق باستخدام الأدوات. تعلم من الصفقات السابقة. "
                ...
                "هذه الحدود غير قابلة للتفاوض ولا يمكنك تجاوزها مهما كتبت من تبريرات.")
    try:
        ...
        try:
                    ...
                    if (pos["direction"] == "Long" and (cp <= pos["sl"] or cp >= pos["tp"])) or \
                       (pos["direction"] == "Short" and (cp >= pos["sl"] or cp <= pos["tp"])):
                        ...
                        send_telegram(f"🛑 [Auto Close]\nSymbol: {sym}\nPnL: ${res.get('pnl', 0):.2f}\n"
                                     f"Equity: ${res.get('equity_after', 0):.2f}\nEmotional Score: {res.get('emotional_score_after', 0)}")
            ...
            if ping % 20 == 0:
                ...
                if s:
                    ...
                    mode = "🟢 عادي" if s.get("risk_mode") != "reduced" else "🟡 مخفّض (Drawdown Protection)"
                    flag = "" if data_complete else " (⚠️ بيانات جزئية)"
                    send_telegram(f"📊 [AATA V7.7]\nCash: ${s['portfolio']['cash']:.2f}\nEquity: ${equity:.2f}{flag}\n"
                                 f"Peak: ${s.get('peak_equity', INITIAL_CAPITAL):.2f}\nRisk Mode: {mode}\n"
                                 f"Emotional: {emo_state} (Score: {emo_score})\nMax Risk Now: {effective_max*100:.1f}%")
        except Exception as e:
...
@app.route('/')
def home(): return "AATA V7.7 - Secure Edition (Keys via Env)!", 200
